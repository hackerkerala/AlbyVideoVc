    ...
    with suppress(MessageIdInvalid, MessageNotModified):
        ...
        if message.command[0] == "fplay":
            if not (message.from_user is None and message.sender_chat or message.from_user.id in admins):
                ...
                await delete_messages([message, k])
                return
        msg = await message.reply_text("⚡️ **Checking recived file..**")
        if message.reply_to_message and message.reply_to_message.video:
            await msg.edit("⚡️ **Checking for Telegram Media...**")
            type='video'
            ...
            if match:
                ...
                """if Config.IS_VIDEO:
                    ...
                    if not width or \
                        not height:
                        await msg.edit("This is an invalid link, provide me a direct link or a youtube link.If it is a error repoty it to @i_am_albin_praveen")
                        await delete_messages([message, msg])
                        return """
                #else:
                try:
        ...
        if type in ["video", "audio"]:
            ...
            try:
                ...
                LOGGER.error(e)
                await msg.edit(
                    f"YouTube Server Download Error ❌\nError:- {e}"
                    )
                LOGGER.error(str(e))
                await delete_messages([message, msg])
                return
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/i_am_albin_praveen')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        await m.reply("", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
